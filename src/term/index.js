...
(function ($) {
  ...
  jQuery(document).on("click", "input.select-img", function (event) {
    ...
    custom_theme_file_frame = wp.media.frames.custom_theme_file_frame =
      wp.media({
        button: {
  ...
  $(".image-upload-button").click(function () {
    ...
    var imgContainer = $(this)
        .closest(".image-fields-wrap")
        .find(".thumbnail-image .image-container"),
      removeimg = $(this)
        .closest(".image-fields-wrap")
    ...
    frame.on("select", function () {
      ...
      imgContainer.html(
        '<img src="' + attachment.url + '" style="width:200px;height:auto;" />'
      );
  ...
  $(document).ajaxSuccess(function (e, request, settings) {
    ...
    if (typeof object == "string") {
      ...
      if (
        object.includes("action=add-tag") &&
        object.includes("screen=edit-category") &&
        object.includes("taxonomy=category")
      ) {
